...
w = np.array([1/3, 1/3, 1/3]) #if weights start equal
#w = np.random.random(3) #if weights start random
window_size = 20
...
for run in range(model_runs):
    ...
    ##averaging previous loss
        ...
        if len(loss_history[chosen_task]) > 1:
           reward = abs(np.mean(np.diff(loss_history[chosen_task])))  #reward is the mean progress
        else:
           reward = 0

#TITLE: GRID SEARCH FOR CRITERIA WEIGHTS
#IMPORT PACKAGES
...
grid_range = range(0, 11) #values from 0 - 10
total_weight = 10
...
##save data
def save_data(data, w1, w2, w3, w4): 
    data.to_csv(f"data/gridsearch_results/gridsearch_{w1}_{w2}_{w3}_{w4}.csv")
...
###############################################################################
#MAIN LOOP
def complete_run(weight_task, parameters, w1, w2, w3, w4):
...
for w2 in grid_range:
    for w3 in grid_range:
        
        #the 4th weight depends on the others (fixed total)
        w4 = total_weight - w1 - w2 - w3
        
        #check and skip negative weights
        if w4 < 0:
            continue
        ...
        complete_run(weight_task, parameters, w1, w2, w3, w4)
